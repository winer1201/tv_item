...
var imgInstance = {
    ...
    flushimg:function(){
        var that = imgInstance;
        if (!that.imgarray || that.imgarray.length <= 0)
        {
            return;
        }
        var html = "";
        html = "<img src='" + that.imgarray[that.imgindex-1] + "' class='page-img-show' >";
        document.all("imgdiv").innerHTML = html;
        that.showdirect();
    },
    showdirect:function(){
        ...
        var showright = false;
        var showleft = false;
        if (that.imgarray && that.imgarray.length > 1) {
            if (that.imgindex <=1) {
                showright = true;
            }
            else if (that.imgindex == that.imgarray.length)
                showleft = true;
            else
            {
                showright = true;
    ...
    move: function (key) {
        var that = imgInstance;
        if (!that.imgarray || that.imgarray.length <= 0) return;
        if (key == tp_move_key.left)
        {
            ...
            that.flushimg();
            //记录用户行为
            var detail = "imgindex[" + that.imgindex + "]_imgurl[" + that.imgarray[that.imgindex - 1] + "]";
            that.actionlog("左移", detail);
        }
        else if (key == tp_move_key.right)
        {
            if (that.imgindex >=that.imgarray.length)
                return;
            that.imgindex += 1;
            that.flushimg();
            //记录用户行为
