...
var arr_num=[],code=[],focu_num=[],pian=[];
//一级页面数据处理
var indexInstance = {
    backUrl: "",
    curNavCode: null,
    ...
    init: function () {
        ...
        that.num_pian=tp.util.getQueryString("pian");
        ...
        //获取天气信息
		var weather=config.cbs_user_getweather;
			weather=tp.util.appendParam(weather, "usercode", m_usercode);
			weather=tp.util.appendParam(weather, "user_session", m_user_session);
			//获取天气信息
        tp.ajax({
            url: weather,
        ...
        tp.ajax({
            ...
            success: function (data) {
            	var res=JSON.parse(data);
            	code.push(res.msgtool);
            	code.push(res.toolbar);
            	if(code[0]==1){
    ...
    loadWeather:function(res){
  		if(res.data && res.retcode==0){
  			$('#tian_num').html(res.data.statusdesc)
  			var num_tian=parseInt(res.data.statuscode);
  			switch(num_tian){
  				case 1:$('#weather').css('background','url(/app/Home/Public/img/ToolBar/qing.png) no-repeat');
  				break;
  				case 2:$('#weather').css('background','url(/app/Home/Public/img/ToolBar/tian.png) no-repeat');
  				break;
  				case 3:$('#weather').css('background','url(/app/Home/Public/img/ToolBar/leizhenyu.png) no-repeat');
  				break;
  				case 4:$('#weather').css('background','url(/app/Home/Public/img/ToolBar/yu.png) no-repeat');
  				break;
  				case 5:$('#weather').css('background','url(/app/Home/Public/img/ToolBar/snow.png) no-repeat');
  				break;
  				case 6:$('#weather').css('background','url(/app/Home/Public/img/ToolBar/yin.png) no-repeat');
  				break;
  				default:return;
  			}
    ...
    enter: function (ctr, key) {
		...
		callback = tp.util.appendParam(callback, "pian", pian[0]);
        ...
        targetUrl = tp.util.appendParam(targetUrl, "areacode", m_areacode);
...
var IndexEvent = {
    ...
    initTable: function (defaultid) {
        var that = IndexEvent;
       /* arr_num.push(that.defaultid.substring(2));
        console.log(arr_num)*/
    ...
    nav_noMove: function (attr) {
        var that = IndexEvent;
        if(attr =="data-up" && code[1]==1){
        	var ctr=document.getElementById(focu_num[0]);
        	if (!ctr) return;
	        var backUrl = "", targetUrl = "", callback = "", id = "";
	        targetUrl = config.portal_toolbar_url;
	        if (!targetUrl || targetUrl == "") return;
	        id = ctr.getAttribute("id");
            backUrl = encodeURIComponent(indexInstance.backUrl);
            callback=config.portal_home_url;
            callback = tp.util.appendParam(callback, "usercode", m_usercode);
	        callback = tp.util.appendParam(callback, "user_session", m_user_session);
	        callback = tp.util.appendParam(callback, "websitecode", m_websitecode);
			...
			callback = tp.util.appendParam(callback, "focusid", id);
			callback = tp.util.appendParam(callback, "pian", pian[0]);
			callback= tp.util.appendParam(callback, "areacode", m_areacode);
			var newreach_token = tp.util.getQueryString("newreach_token");
	        if (newreach_token && newreach_token != "") {
	            callback = tp.util.appendParam(callback, "newreach_token", newreach_token);
	            targetUrl = tp.util.appendParam(targetUrl, "newreach_token", newreach_token);
    ...
    nav_Changed: function (lastNav, curNav) {
		...
		var num_nav=document.getElementsByClassName('navbar');
    ...
    noMove: function (attr, ctrType) {
        ...
        {
        	if (ctrType == tp_enum_ctrType.table) {
	            if (attr == tp_enum_moveData.up && code[1]==1) {
	            	var ctr=document.getElementById(focu_num[0]);
	            	if (!ctr) return;
			        var backUrl = "", targetUrl = "", callback = "", id = "";
			        targetUrl = config.portal_toolbar_url;
    ...
    selid:function(id){
       ...
       //console.log(id);
        //console.log(IndexEvent.defaultid.substring(2))
    	var left=$('#'+id).offset().left;
   		if(arr_num[0] == tp_move_key.right){
   			
			var distance=1280-left;
			if(distance<$('#'+id)[0].offsetWidth){
				var lf=parseInt($('table').css('left'));
				$('table').css('left',lf-320);
			}
   		}
       	...
       	arr_num=[];
       	focu_num=[];
       	var lt_pian=parseInt($('table').css('left'));
       	pian=[];
       	pian.push(lt_pian);
       	/*存储id给nomove使用*/
      	focu_num.push(id);
