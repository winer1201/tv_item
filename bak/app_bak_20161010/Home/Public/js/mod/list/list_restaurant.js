...
var listRestaurant = {
    ...
    enter: function (key) {
        var that = listRestaurant;
        var ctr = that.tabCtr.curCtr;
        if (!ctr) return;

        var content_code = "", contentcode = "", reststatus = "";
        var imglist = "", pagecode = "", areacode = "";
        var backUrl = "", targetUrl = "", targetcode = "", callback = "";

        reststatus = ctr.getAttribute("status");
        if (reststatus != "0") {
            //餐厅未营业
            return;
        }
        ...
        content_code = ctr.getAttribute("contentcode");
        ...
        contentcode = tp.util.getQueryString("contentcode");
        backUrl = tp.util.getQueryString("backUrl");
        backUrl = encodeURIComponent(backUrl);

        callback = config.portal_restaurant_list_url;
        callback = common.setBaseParam(callback);
        callback = tp.util.appendParam(callback, "contentcode", contentcode);
        callback = tp.util.appendParam(callback, "pageIndex", that.pageIndex);
        callback = tp.util.appendParam(callback, "focusid", that.tabCtr.curid);
        ...
        //记录用户行为
        that.actionlog("点击", "餐厅图标_[" + targetcode + "]");
    },
    esc: function (key) {
        var that = listRestaurant;
        //返回上一页
    ...
    actionlog: function (action, detail) {
        ...
        ActionHelper.record({
            ...
            action: action,
            page: "列表页" + "[周边餐厅]_页码[" + that.pageIndex + "]",
            detail: detail
        });
    }
};